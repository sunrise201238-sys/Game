...
const MICRO_RIFT_WIDTH = 800;
const MICRO_RIFT_HEIGHT = 450;
const scaleRiftX = (value: number) => Math.round((value / RIFT_WIDTH) * MICRO_RIFT_WIDTH);
const scaleRiftY = (value: number) => Math.round((value / RIFT_HEIGHT) * MICRO_RIFT_HEIGHT);
const scaleRiftRect = (rect: Rect): Rect => ({
  x: scaleRiftX(rect.x),
  y: scaleRiftY(rect.y),
  width: Math.max(1, scaleRiftX(rect.width)),
  height: Math.max(1, scaleRiftY(rect.height)),
});
const scaleRiftPoint = (point: XY): XY => ({
  x: scaleRiftX(point.x),
  y: scaleRiftY(point.y),
});
const microRiftLakes = buildRiftLakes(12).map(scaleRiftRect);
const microRiftWalls = [...riftEdgeWalls, ...riftDiagonalWalls].map(scaleRiftRect);
const microRiftPlayerSpawns = riftPlayerSpawns.map(scaleRiftPoint);
const microRiftBotSpawns = riftBotSpawns.map(scaleRiftPoint);
const MICRO_RIFT_BASE_BL = scaleRiftPoint(RIFT_BASE_BL);
const MICRO_RIFT_BASE_TR = scaleRiftPoint(RIFT_BASE_TR);
...
const MAPS_UNSORTED: MapDefinition[] = [
  ...
  },
  {
    id: 'micro-rift',
    name: 'Micro Rift',
    description: 'Miniature Rift with the same cross-pass geometry and corner-base objective.',
    width: MICRO_RIFT_WIDTH,
    height: MICRO_RIFT_HEIGHT,
    lakes: microRiftLakes,
    walls: microRiftWalls,
    playerSpawns: microRiftPlayerSpawns,
    botSpawns: microRiftBotSpawns,
    vipUnits: [
      { team: 0, position: MICRO_RIFT_BASE_BL, unitId: 'base' },
      { team: 1, position: MICRO_RIFT_BASE_TR, unitId: 'base' },
    ],
  },
  {
    id: 'the-rift',
