...
- Drag-to-aim Canvas client with immediate local playback of submitted actions synced to server diffs.
...
- Thin WebSocket server handling matchmaking, alternating turn timers, authoritative resolution, and reconnects.
...
| `PLANNING_MS`  | `7000`               | Turn timer for PvP matches (ignored for PvE games)      |
...
- Matchmaking proceeds through alternating action submissions followed by server `ROUND_RESULT` playback.
...
4. Recommended environment variables:
   - `PLANNING_MS=7000` (tune the PvP turn timer; ignored for PvE bot games).
   - `ALLOW_ORIGINS=https://<your-client>.onrender.com`.
   - `ANALYTICS=off` or `ANALYTICS=minimal`.
